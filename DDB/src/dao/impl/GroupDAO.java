...
/**
 * A data access object (DAO) providing persistence and search support for Group entities. Transaction control of the save(), update() and delete() operations can directly support Spring container-managed transactions or they can be augmented to handle user-managed Spring transactions. Each of these methods provides additional information for how to configure it for the desired type of transaction control.
 * 
 * @see domain.Group
...
public class GroupDAO extends HibernateDaoSupport {
	...
	public Group findById(java.lang.Integer id) {
		...
		try {
			Group instance = (Group) getHibernateTemplate().get("domain.Group",
					id);
			return instance;
		} catch (RuntimeException re) {
